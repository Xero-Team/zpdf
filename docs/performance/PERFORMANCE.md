...
所以这一项不会改变 §15.3 的表）。实现需要：`FontCache` 改存 `Arc<LoadedFont>`
（否则跨页复用要深拷贝，而 CJK 字体的 `cid_to_gid`/`to_unicode` 都是 HashMap，拷贝
等于重解析）、`PdfDocument` 持文档级 `ObjectId → Arc<LoadedFont>`、以及把多页消费者
（CLI 的 6 处 `load_page_fonts`、viewer、bench 的 longform）改成复用同一份缓存。
**输出不变（同一份已解析字体），所以是字节等价改动**，但影响面在调用方，属"要不要做"
的决定。
...
| ②逐页字体装载 | **已诊断**：页内无杠杆；文档级缓存 40–90%（§15.7），需调用方改造的决定 |
