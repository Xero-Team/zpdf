//! EUC-KR / UHC (Windows code page 949) 2-byte code -> Unicode (BMP) lookup table.
//!
//! Used to decode the predefined Adobe byte-encoded CMaps
//! (KSC-EUC-H/V, KSCms-UHC-H/V, KSCms-UHC-HW-H/V, KSCpc-EUC-H) for non-embedded CJK fonts: the 2-byte code is
...
//! `cp949` codec). Sorted by `code` so it is binary-searchable.

/// `(code, unicode)` pairs, sorted ascending by `code` (big-endian
/// `lead << 8 | trail`).
#[rustfmt::skip]
pub static KSC_TO_UNICODE: &[(u16, u16)] = &[
...
/// Decode a 2-byte EUC-KR / UHC (Windows code page 949) code to a Unicode scalar, or `None` if unmapped.
pub fn ksc_to_unicode(code: u16) -> Option<char> {
    KSC_TO_UNICODE
        .binary_search_by_key(&code, |&(c, _)| c)
        .ok()
        .and_then(|i| char::from_u32(KSC_TO_UNICODE[i].1 as u32))
}
