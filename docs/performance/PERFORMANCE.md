...
| **2** | **interpret 阶段**（新） | 占流水线 10–47%；test12 **397 ms** 且 DPI 无关；其中 shading 栅格化只在 interpret 可见 | **已定位（§15）：拆成图像解码 + 逐页字体装载** |
...
## 15. interpret 阶段定位（2026-09-12）— 候选 2 拆成了两个

### 15.1 起点：一个只有总数的黑盒

候选 2（interpret）在 §13.5 里排在候选 1 之后，理由是"占流水线 10–47%，DPI 无关"。
但 `InterpretStats` 当时只有 `total_ns` 与 `shading_ns` 两个字段 —— 对 test12
（377 ms）这种 shading≈0 的页，它就是个黑盒。**"interpret 慢"和 §13.8 之前的
"soft-mask 占 69%" 一样不可执行**，所以这一步只做定位。

### 15.2 方法

- `InterpretStats` 增加六个**分类归属桶**（path / paint / text / image / form / color）
  与六个确定性计数器，外加 `describe()`（CLI、bench 与未来的消费者用同一段格式化，
  和 wgpu 侧的 `AdapterIdentity::describe` 同构）。
- **桶是归属，不是划分**：嵌套的表单/遮蔽组内容整体算进 `form_ns`，因此不再按分类重复归属
  （表单内部的操作符循环刻意不插桩），"未计入"的余量是分词与那些不值得读时钟的操作符
  （`q`/`Q`/`gs`/`cm` —— 向量页上的操作符绝大多数是它们）。
- **分类计时是 opt-in**（`with_work_timing`）：这些时钟落在**每个被归属的操作符**上，
  test11 这样的页有 4057 个 path 操作，按渲染侧那套理由（"时钟不能进每图元循环"）必须可选。
  整页与 shading 的计时仍然无条件采集。
- **把一个混淆拆开了**：`stages/interpret` 量的是"装载 + 解释"，而
  `InterpretStats::total_ns` 只包住解释。给 `PageSetup` 加了 `setup_ns`
  （字体装载 / 内容字节 / 注解 / OC / 输出意图 / 缓存构建），于是"interpret"这个词
  终于能指一个确定的东西。

### 15.3 实测（150 DPI；总量取自 criterion `stages/interpret` 中位数，拆分取自
`ZPDF_BENCH_DEBUG=1` 的单次插桩运行；同为 release）

| 页 | criterion interpret | 其中 setup | 内容解释 | 最大桶 |
|---|---|---|---|---|
| test12 | 377.5 ms | 2.3 | **346.9** | **image 331.1（95%）** |
| testpdf-ai | 74.7 | 0.9 | **71.8** | **shading 61.8（86%）** |
| test1 | 15.8 | **13.1** | 0.39 | text 0.13（34%） |
| test11 | 10.1 | **10.2** | 1.20 | text 0.23 / path 0.14 |
| test7 | 6.9 | 0.04 | 6.21 | form 6.20（100%） |
| test2 | 4.5 | 1.8 | 2.18 | form 2.09（96%） |
| test4 | 4.3 | 0.04 | 4.03 | **image 4.02（100%）** |
| test8 | 3.9 | 2.7 | 1.30 | text 0.37 / image 0.41 / color 0.31 |
| test10 | 2.2 | 1.3 | 0.75 | text 0.25 / color 0.32 |

setup 的成分（单独量，release）：test1 **字体 13.17 ms（96 个字体）**、
test11 **字体 10.75 ms（12 个字体，约 0.9 ms/字体）**、testpdf-ai 0.87 ms（18 个）、
test7 0.00 ms（0 个字体）—— 注解与输出意图都 <0.02 ms，所以 setup ≈ 字体装载。

### 15.4 结论：候选 2 不是一个候选，是两个

1. **图像解码（新第一名）** —— test12 的内容解释 346.9 ms 里 **331.1 ms 是解码**
   （16 次 image op，约 20 ms/张）。对照 testpdf-ai：13 张图只花 8.6 ms（约 0.66 ms/张）
   —— 同一个解码路径上 30 倍的单张差距，说明 test12 的图走进了一条昂贵分支
   （尺寸、色彩空间或编码格式），而不是"图像本来就慢"。按整页流水线算，
   解码占 test12 的 ~51%。**下一步就是定位这 20 ms/张花在哪**：解码后的
   RGB→RGBA/预乘转换、全分辨率解码后再降采样（渲染侧已有 box 降采样缓存，
   见候选 4）、还是某类 JPEG 的特殊路径。
2. **逐页字体装载（新第二名）** —— test1 的"interpret 15.8 ms"里 13.1 ms 是装载
   96 个字体；test11 的 10.1 ms 里 10.8 ms 是 12 个字体。按整页流水线，test1 约 40%、
   test11 约 30% 花在这里。`load_page_fonts` **每次调用都重新解析**（同一页连续三次
   13.0/13.1/13.2 ms，无记忆化），而文档的多页往往共享同一批字体对象
   （test1 p0 列 96 个、p1 列 18 个）。可动的方向：文档级已解析字体缓存（按字体对象）
   与/或按需解析（页面资源里列了但没被 `Tf` 选中的字体不必解析）。
3. **shading 栅格化**（原候选 2 的真实成分）—— testpdf-ai 61.8 ms，占该页流水线 ~31%，
   只在 interpret 可见（§13.1 的设计理由）。现在它有了明确归属，不再藏在"interpret"里。
4. render 侧剩余项（§14.7 之后）：图像重采样（test12 render 里 94 ms / testpdf-ai 38 ms）、
   遮蔽面 render、clip（test7 的 920 次 push）。

### 15.5 顺带落地：mask 平面 reduce 也收窄（同一批机制）

遮蔽面是整页大小的，但组没画到的地方只可能是"未绘制值"（流明遮蔽是 /BC 亮度，
alpha 遮蔽是透明），而 `/TR` LUT 对该值的作用恰好就是 `unpainted_value(mask)`。
于是 reduce 改为：先用该值填满平面，只对**子渲染器记录的已绘制 region**逐像素计算
（这正是 §14 那套记账的复用 —— 子渲染器的 `dirty` 本来就是组画过的保守超集）。

| 页 | reduce（前） | reduce（后） | 处理像素 | 未收窄时 | leak |
|---|---|---|---|---|---|
| test12（8 个遮蔽） | 38.2–38.7 ms | **5.4–6.2 ms** | 2,096,494 | 18,000,000 | **0** |
| testpdf-ai（1 个遮蔽） | 5.7–6.1 ms | 5.6–6.9 ms | 2,250,000（该组覆盖整页，无可收窄） | 2,250,000 | **0** |

**leak 判据在这里自己出过一次错，值得记下来**：第一版把 leak 写成"region 之外
alpha≠0 的像素数"（照搬 §14 的合成检查），实测 test12 报出 **14,733,506** 个"泄漏"。
不是收窄错了 —— 是判据错了：**流明遮蔽的目标栅格一开始就是 /BC 不透明底色**，
每个像素的 alpha 都非 0。正确的判据是"region 之外的像素**归约后的值**是否等于
`unpainted_value`"，也就是这个捷径真正依赖的不变量。判据改成值比较后，
真实页与测试（**两种遮蔽类型各一例**）都是 0。共享一次归约实现
（`mask_plane_value`）保证"检查"与"被检查的代码"不会各自漂移。

新增计数器：`mask_reduce_px`（实际归约的平面像素）、`mask_reduce_leak_px`（恒为 0）。
验证：25 页 PNG 字节级一致、GPU oracle 三套通过、618 个 failed 语料逐文件状态不变。

### 15.6 图像解码：先落字节等价的一半，剩下的要决策

按 15.4 的顺序先做第一名（图像解码）。临时探针（已删）把 test12 p0 **一张**
2550×3300 YCCK JPEG（8.4 Mpx）的解码拆开：

| 阶段 | 耗时 |
|---|---|
| zune-jpeg DCT 解码（IDCT + 4 分量上采样） | 25.4 ms |
| **`ycck_to_rgb`（本仓库自己的逐像素 f64 转换）** | **121.4 ms** |
| 样本 → RGBA（`decode_raw_samples`） | 34.5 ms |
| `/SMask` 折叠（`apply_smask_image`） | 53.0 ms |

即"filter 解码 152 ms"里 **121 ms 不是第三方解码器，而是我们自己的转换**：逐像素
YCbCr→CMYK（在 `zpdf-parser` 里，因为 Adobe YCCK 需要按 Adobe 多项式处理）→ 再经
Adobe 多项式转 RGB。

**已落地（字节等价，无需决策）**：两个热循环
- `decode_raw_samples`：8 位样本且无 colour-key 时走专用路径 —— 直接字节读取、预分配
  输出缓冲、按行切片，取代每像素走一次位读取器、每像素一次 `extend_from_slice`；
- `apply_smask_image` / `multiply_pixel_alpha`：定长 `[u8; 4]` + `alpha == 255` 提前返回
  （`mul255(v, 255) == v` 恒成立，两种写法逐字节相同）。

实测（150 DPI，interpret **内容**阶段）：

| 页 | 前 | 后 | image 桶 |
|---|---|---|---|
| test12 | 346.9 ms | **294.6** | 331.1 → **277.5** |
| testpdf-ai | 71.8 | 63.6 | 8.6 → 6.9 |
| test4 | 4.03 | 3.72 | 4.02 → 3.70 |

验证：25 页 PNG 字节级一致、GPU oracle 三套通过、618 个 failed 语料逐文件状态不变。

**剩下的两件事都需要决策，不是纯性能改动**：

1. **SIMD 化 `ycck_to_rgb`**。Adobe 多项式单独实测 **63.2 ms / 8.4 Mpx = 7.5 ns/px
   ≈ 4.5 flops/cycle —— 已经贴着标量 f64 FMA 吞吐上限**（约 4 flops/cycle），所以
   "换个写法/少几次运算"没有空间（重结合或 f32 会改变舍入，破坏字节等价）。
   唯一更快的方式是向量化（lane 内操作顺序不变 ⇒ 结果位级相同 ⇒ 仍是字节等价），
   代价是本仓库将出现**第一处 `unsafe` 与架构相关代码**（当前 `crates/*/src` 里 0 处
   unsafe、0 处 `target_arch` 分支），并需要一个标量回退给 wasm。
2. **降分辨率解码**。8.4 Mpx 解码出来后，渲染侧在 150 DPI 只用到约 0.5 Mpx（box 降采样
   由渲染器做）。按 DCT 比例（1/2、1/4）解码可把整条链降 4–16 倍，但输出像素
   **不再字节相同**：这是保真度取舍，需要 opt-in 或默认值上的决定。GPU↔CPU oracle 的
   容差（<1% 像素、16 级）大概率仍能通过，但"像素等价"这条纪律会在这里第一次让步。

### 15.7 逐页字体装载：没有页内捷径；杠杆在"文档级"

先否证一个假设：**懒解析（只解析被 `Tf` 选中的字体）没有收益** —— 语料上"列出的字体"
与"被 `Tf` 选中的字体"**完全相等**（test1 p0 96/96、test11 p0 12/12、test8 5/5、
test10 7/7），这些页确实用满了它列出的字体。

再拆成本：ToUnicode/编码等"文本映射"只占每字体 **0.014–0.085 ms**（约 10–20%），
其余是字体程序解析本身（0.04–0.37 ms/字体，均匀分布，无单点）。所以**页内没有安全的
大杠杆**。

真正的杠杆是**文档级缓存已解析字体**——因为各页大量共享同一批字体对象：

| 文档 | 页数 | 列出总数 | **不同字体对象** | 逐页重复解析合计 | 缓存后（估） |
|---|---|---|---|---|---|
| test1 | 16 | 1260 | **197** | 207.9 ms | ~32 ms（−85%） |
| test11 | 12 | 99 | **12** | 120.8 ms | ~15 ms（−88%） |
| test8 | 44 | 471 | 279 | 124.5 ms | ~74 ms（−40%） |
| test10 | 73 | 136 | **13** | 44.0 ms | ~4 ms（−90%） |

按 **文档**算这是 40–90% 的字体装载，但按**单页**算 0%（campaign 的矩阵量的正是单页，
所以这一项不会改变 §15.3 的表）。

**已落地（字节等价，调用方无改动）**：
- `FontCache` 改存 `Arc<LoadedFont>`（`insert` 语义不变，新增 `insert_shared` /
  `try_insert_shared_with_limit`）。不这样做的话跨页复用要深拷贝，而 CJK 字体的
  `cid_to_gid`/`to_unicode` 都是 HashMap，拷贝成本等于重解析。
- `PdfDocument` 新增文档级 `ObjectId → Arc<LoadedFont>` 缓存（`SharedFonts`），
  `load_page_fonts` 命中即复用。`LoadedFont` 构建后不可变，所以共享是安全的；
  预算沿用 `max_font_cache_bytes`，超预算后回到逐页解析 —— 字体极多的文档退化为今天的行为，
  而不是无界增长。
- 每页仍有自己的 `FontCache`（名字 → `FontId` 仍是页内语义），共享的是**已解析的字体本身**，
  因此 CLI/viewer/bench 都不用改。

实测（依次装载整个文档每一页的字体，release）：

| 文档 | 页数 | 不同字体对象 | 前 | 后 | 变化 |
|---|---|---|---|---|---|
| test1 | 16 | 197 | 207.9 ms | **38.5** | −81% |
| test11 | 12 | 12 | 120.8 ms | **12.4** | −90% |
| test8 | 44 | 279 | 124.5 ms | **63.3** | −49% |
| test10 | 73 | 13 | 44.0 ms | **3.3** | −92% |

单页运行不变（第一页照样完整解析），所以这是**多页 / 阅读器路径**的收益，不是 §15.3 的
单页矩阵。新增测试：同一文档两页引用同一字体对象时，第二次拿到的是同一个已解析字体
（指针相同，且文档级只解析一次）。验证：25 页 PNG 字节级一致、GPU oracle 三套通过、
618 个 failed 语料逐文件状态不变。

### 15.8 shading 栅格化：已经在标量 f64 的地板上

`zpdf-content/shading.rs` 的逐像素循环：反变换（4 次 f64 乘）+ `param_at`
（轴向：2 乘 1 **除**；径向：2 次 **sqrt** + 2 除）+ LUT 采样 + 3 次 clamp/round/存。
实测 testpdf-ai 13 次栅格 61.8 ms，单次约 4.75 ms；按满页 ~2.25 Mpx 推算约
**2–4 ns/像素**，与上面这份 f64 运算量的标量吞吐一致（同 §15.6 的结论：换写法
——把常数除法提到循环外、按行增量递推 `t`、合并两次仿射——都会改变舍入，破坏字节等价；
唯一更快的路是 SIMD）。**结论：shading 没有字节等价的快捷优化**，与 §15.6 的第 1 条
共用一个决定。
...
| 候选 | 状态 |
|---|---|
| ①图像解码 | **已落地一半**（§15.6，字节等价，test12 −52 ms）；余下 `ycck_to_rgb`(121 ms) 需 SIMD 决定 |
| ②逐页字体装载 | **已落地**（§15.7，文档级共享已解析字体，多页 −49%~−92%，单页不变） |
| ③shading | **已诊断**：已在标量地板（§15.8），与 ① 同属 SIMD 决定 |
| ④render 侧剩余（图像重采样 / 遮蔽面 render / clip） | 未动。clip 那一项补了一条测量：test7 p0 的显示列表里有 **1324 条 `PushClip`，其中 1323 条的几何**互不相同**（逐元素的小矩形），所以"缓存同款 mask"没有收益；渲染器报告实际 push 成功 **920** 次 —— 恰好等于 `MAX_CLIP_PIXEL_WORK`(2e9 px) ÷ 页面积(2.17 Mpx)，即该页后半程的 clip 已被预算跳过（§13.1 记过的"略微过绘"降级）。真正的杠杆是矩形 clip 的快路径，而 tiny-skia 的裁剪必须走 `Mask`，因此这属于"要换机制"的改动 |

即：**三个"更快的算法"候选都被同一堵墙挡住 —— 逐像素 f64 运算已经在标量吞吐上限，
要么引入 SIMD（本仓库首处 `unsafe` + 架构相关代码），要么接受保真度取舍。**
