...
use crate::transform::{PageMap, TexturedVertex};
...
/// the fixed page Y-flip relative to the page rect's top/left edges). See
/// `CpuRenderer::render_image` for the derivation.
pub fn image_quad(iw: f32, ih: f32, tm: &Matrix, map: &PageMap, alpha: f32) -> [TexturedVertex; 4] {
    let s = map.scale;
    let (a, b, c, d, e, f) = (
        tm.a as f32,
        tm.b as f32,
...
/// (group 1: texture + the shared bilinear sampler). Bytes are uploaded verbatim —
/// no host premultiply (the shader/blend reproduce the CPU's compositing).
pub fn upload_image_bind_group(ctx: &GpuContext, img: &DecodedImage) -> wgpu::BindGroup {
